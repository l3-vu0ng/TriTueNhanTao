...
# ══════════════════════════════════════════════════
class NodeInfo:
    def __init__(self, state, action=None, depth=0, cost=0, parent_label=None, label='?', parent=None):
        self.state        = state
        self.action       = action
        self.depth        = depth
        self.cost         = cost
...
def count_correct_positions(state, goal_state):
    count = 0
    for r in range(3):
        for c in range(3):
...
#  UCS – FULL TRACE
# ══════════════════════════════════════════════════
def ucs_full_trace(initial, goal):
    goal_t  = tuple(tuple(r) for r in goal)
    label_i = [0]

    def make_node(state, action=None, depth=0, cost=0, parent_node=None):
        parent_label = parent_node.label if parent_node else None
        n = NodeInfo(state, action, depth, cost, parent_label, LABELS[label_i[0]], parent=parent_node)
        label_i[0] += 1
        return n

    steps = []
    start = make_node(initial, cost=0)

    # ── Step 0 : init ─────────────────────────────
    frontier_q    = []
    counter       = itertools.count()
    heapq.heappush(frontier_q, (start.cost, next(counter), start))
    
    frontier_map  = {start.state: start.cost}
    explored_list = []
    explored_set  = set()

    steps.append(UCSStep('init', None,
                         [n for _, _, n in sorted(frontier_q)], [],
                         {start.label},
                         f'Khởi tạo: thêm [{start.label}] (trạng thái ban đầu) vào Frontier'))

    # ── UCS loop ──────────────────────────────────
    while frontier_q:
        current_cost, _, node = heapq.heappop(frontier_q)

        # Skip stale entries that were replaced by cheaper paths
        if node.state in explored_set:
            continue
            
        # Clean up frontier_map since we are exploring it
        frontier_map.pop(node.state, None)

        # GOAL-TEST on dequeue
        if node.state == goal_t:
            explored_list.append(node)
            steps.append(UCSStep('found', node,
                                 [n for _, _, n in sorted(frontier_q)],
                                 list(explored_list),
                                 set(),
                                 f'Dequeue [{node.label}] (Cost={node.cost}) — đây là GOAL! Tìm thấy lời giải!'))
            return steps
        ...
        for dir_name, dr, dc in MOVE_DIRS:
            ...
            correct_tiles = count_correct_positions(child_state, goal_t)
            children_candidates.append({
                'state': child_state,
                'action': dir_name,
                'correct': correct_tiles
            })
            
        # Sort candidates descending by correct_tiles
        children_candidates.sort(key=lambda x: x['correct'], reverse=True)

        current_rank = 0
        last_correct = None
        for cand in children_candidates:
            if cand['correct'] != last_correct:
                current_rank += 1
                last_correct = cand['correct']
                
            step_cost = current_rank
            child_state = cand['state']
            dir_name = cand['action']
            child_cost = node.cost + step_cost
            
            if child_state in explored_set:
                continue
                
            if child_state not in frontier_map or child_cost < frontier_map[child_state]:
                child = make_node(child_state, dir_name, node.depth + 1, child_cost, node)
                frontier_map[child_state] = child_cost
                heapq.heappush(frontier_q, (child_cost, next(counter), child))
                new_labels.add(child.label)

        steps.append(UCSStep('expand', node,
                             [n for _, _, n in sorted(frontier_q)],
                             list(explored_list),
                             new_labels,
                             f'Mở rộng [{node.label}] (Cost={node.cost}): '
                             f'thêm {len(new_labels)} children vào Frontier'))

    steps.append(UCSStep('failure', None, [], list(explored_list), set(),
...
class App:
    def __init__(self, root):
        self.root   = root
        self.root.title('8-Puzzle — UCS Algorithm')
        self.root.configure(bg=BG)
        self.root.geometry('1250x720')
        self.root.resizable(True, True)

        self.steps   = ucs_full_trace(INITIAL, GOAL)
        self.idx     = 0
        self._auto   = False
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text='8-Puzzle — UCS Algorithm',
                 font=('Segoe UI', 16, 'bold'), bg=BG, fg=ACCENT).pack(side='left')
        tk.Label(hdr, text=f'  ({len(self.steps)} bước UCS)',
    ...
    def _build_left(self, parent):
        ...
        self.cur_cost   = tk.Label(pnl, text='', bg=PANEL, fg=GREEN,
                                   font=('Segoe UI', 9))
        self.cur_cost.pack()
    ...
    def _build_center(self, parent):
        pnl = self._panel(parent, 'Frontier — Priority Queue (Min-Cost)')
    ...
    def _show_path(self):
        ...
        top.title(f"Đường đi tới Goal ({len(path)-1} bước) - Chi phí: {found_step.current_node.cost}")
        ...
        for i, node in enumerate(path):
            ...
            tk.Label(frame, text=f"[{node.label}] Cost:{node.cost}", bg=PANEL, fg=ACCENT, font=('Segoe UI', 9)).pack(pady=(0, 5))
    ...
    def _render(self, idx):
        ...
        if step.current_node:
            ...
            self.cur_cost.config(text=f'Cost: {n.cost}')
    ...
    def _draw_frontier(self, step):
        ...
        for i, (kind, node) in enumerate(display):
            ...
            is_ghost = (kind == 'ghost')
            # In UCS, queue is sorted, first element is the next one.
            ...
            if is_ghost:
                c.create_text(tx, ty, text=f'[{node.label}]  ← pop',
                              anchor='nw', fill=GRAY,
                ...
                c.create_text(tx, ty + 28, text=f'Cost: {node.cost}',
                              anchor='nw', fill=ghost_clr, font=('Consolas', 8))
                c.create_text(tx, ty + 42, text=f'Depth: {node.depth}',
                              anchor='nw', fill=ghost_clr, font=('Consolas', 8))
                par = f'Par: [{node.parent_label}]' if node.parent_label else 'Par: —'
                c.create_text(tx, ty + 56, text=par,
                              anchor='nw', fill=ghost_clr, font=('Consolas', 8))
            else:
                if is_next:
                    tag = '  ⏩NEXT'
                ...
                c.create_text(tx, ty + 28, text=f'Cost: {node.cost}',
                              anchor='nw', fill=GREEN, font=('Consolas', 8))
                c.create_text(tx, ty + 42, text=f'Depth: {node.depth}',
    ...
    def _draw_explored(self, step):
        ...
        c.delete('all')
        goal_t = tuple(tuple(r) for r in GOAL)
        item_h = 104       # tăng để chứa thêm dòng Cost, Parent
        pad_x  = 10
        pad_y  = 8
        ...
        for i, node in enumerate(step.explored):
            ...
            c.create_text(tx, ty + 16, text=f'Act : {act}',
                          ...
                          font=('Consolas', 8))
            c.create_text(tx, ty + 30, text=f'Cost: {node.cost}',
                          anchor='nw', fill=GREEN,
                          font=('Consolas', 8))
            c.create_text(tx, ty + 44, text=f'Dep : {node.depth}',
                          anchor='nw', fill=GRAY,
                          font=('Consolas', 8))
            par = f'Par : [{node.parent_label}]' if node.parent_label else 'Par : —'
            c.create_text(tx, ty + 58, text=par,
